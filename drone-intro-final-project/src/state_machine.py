    ...
    def execute(self):
        ...
        if cur == self.States.IDLE:
            ...
            if self.pose_error == None:
                return
            pose_error_arr = point_to_arr(self.pose_error.position)
            angle_error = orientation_to_yaw(self.pose_error.orientation)
            self.pose_error = None
            magnitude = np.linalg.norm(pose_error_arr)
            # check if limit is reached
            if abs(angle_error) < self.max_angle_error and magnitude < self.max_pose_error:
            ...
            self._mav1.set_target_pose(target_pose)
